#include<stdio.h>
#include<pthread.h>
#include<stdlib.h>
#include<unistd.h>

pthread_mutex_t mutex, writeTo;
int sharedVariable, readerCount = 0;

void *writer(void *arg){
        pthread_mutex_lock(&writeTo);
        int randNum = rand() % 10;
        int d = ((int)arg);
        printf("--------------------------------------------------\n");
        printf("W%d Wait for Random time between 0ns and 10ns = %d\n", d, randNum);
        sleep(randNum);
        printf("Enter the number of time W%d want to write:\n", d);
        int t;
        scanf("%d", &t);
        printf("Now W%d is writing... i.e. ADDING...\n", d);
        int j;
        for(j=0; j<t; j++){
                printf("Enter the %dth INTEGER value to write:\n", (j+1));
                int u;
                scanf("%d", &u);
                sharedVariable = sharedVariable + u;
        }
        printf("UPDATED value of Shared variable = %d \n", sharedVariable);
        printf("--------------------------------------------------\n");
        pthread_mutex_unlock(&writeTo);
}

void *reader(void *arg){
        //Entry Part
        pthread_mutex_lock(&mutex);
        readerCount++;
        if(readerCount==1){
                pthread_mutex_lock(&writeTo);//No writer should come
        }
        pthread_mutex_unlock(&mutex);//so next reader can come
        //Exit Part
        int randNum = rand() % 10;
        int d = ((int)arg);
        printf("R%d wait for Random time between 0ns and 10ns = %d\n", d, randNum);
        sleep(randNum);
        printf("Enter the number of time R%d want to read:\n", d);
        int t;
        scanf("%d", &t);
        printf("Now R%d is reading....\n", d);
        int j;
        for(j=0; j<t; j++){
                printf("R%d read the shared value = %d\n", d, sharedVariable);
        }
        printf("Number of Readers present = %d\n", readerCount);
        pthread_mutex_lock(&mutex);
        readerCount--;
        if(readerCount==0){//Now writer can come if they want
                pthread_mutex_unlock(&writeTo);
        }
        pthread_mutex_unlock(&mutex);
}

void main(){
        printf("Enter the initial value of share variable: \n");
        scanf("%d", &sharedVariable);
        printf("---------------------------------------------\n");
        int readersNumber, writersNumber, i;
        printf("Enter the no. of Reader:\n");
        scanf("%d", &readersNumber);
        for(i=0; i<readersNumber; i++){
                printf("R%d\n", i);
        }
        printf("---------------------------------------------\n");
        printf("Enter the no. of Writer:\n");
        scanf("%d", &writersNumber);
        for(i=0; i<writersNumber; i++){
                printf("W%d\n", i);
        }
        printf("---------------------------------------------\n");

        pthread_t r[readersNumber], w[writersNumber];
        pthread_mutex_init(&writeTo, NULL);
        pthread_mutex_init(&mutex, NULL);

        if(readersNumber<0 || writersNumber<0){
                printf("Sorry: You have Entered NEGATIVE number of READER | WRITER\n");
                printf("Program is Terminating....\n");
                return;
        }else if(readersNumber == 0){
                printf("Sorry: You have not taken any READER\n");
                printf("No READER thread will be creaded\n");
        }else if(writersNumber == 0){
                printf("Sorry: You have not taken any WRITER\n");
                printf("No WRITER thread will be creaded\n");
        }else{
                printf("Thread Creating....\n");
        }
        printf("---------------------------------------------\n");

        if(writersNumber==readersNumber){
                for(i=0; i<writersNumber; i++){
                        pthread_create(&w[i], NULL, &writer, (int *)i);
                        pthread_create(&r[i], NULL, &reader, (int *)i);
                }
                for(i=0; i<writersNumber; i++){
                        pthread_join(w[i], NULL);
                        pthread_join(r[i], NULL);
                }
        }else if(writersNumber>readersNumber){
                for(i=0; i<readersNumber; i++){
                        pthread_create(&w[i], NULL, &writer, (int *)i);
                        pthread_create(&r[i], NULL, &reader, (int *)i);
                }
                for(i=readersNumber; i<writersNumber; i++){
                        pthread_create(&w[i], NULL, &writer, (int *)i);
                }
                for(i=0; i<readersNumber; i++){
                        pthread_join(w[i], NULL);
                        pthread_join(r[i], NULL);
                }
                for(i=readersNumber; i<writersNumber; i++){
                        pthread_join(w[i], NULL);
                }
        }else{
                for(i=0; i<writersNumber; i++){
                        pthread_create(&w[i], NULL, &writer, (int *)i);
                        pthread_create(&r[i], NULL, &reader, (int *)i);
                }
                for(i=writersNumber; i<readersNumber; i++){
                        pthread_create(&r[i], NULL, &reader, (int *)i);
                }
                for(i=0; i<writersNumber; i++){
                        pthread_join(w[i], NULL);
                        pthread_join(r[i], NULL);
                }
                for(i=writersNumber; i<readersNumber; i++){
                        pthread_join(r[i], NULL);
                }
        }
        printf("-------------After joining the thread---------\n");
        printf("Final value of share variable = %d\n", sharedVariable);
}
